...
function shouldFormatOnType(change, editor) {
  ...
  if (change.newText.length > 1 && !isBracketPair(change.newText, editor)) return false;
  return true;
}
...
function isBracketPair(text, editor) {
  if (lumine.packages.getActivePackage("bracket-matcher") === undefined) return false;
  const pairs = lumine.config.get("bracket-matcher.autocompleteCharacters", {
...
module.exports = class CodeFormatManager {
  constructor() {
    ...
    this.subscriptions = new CompositeDisposable(
      lumine.commands.add("lumine-text-editor:not([mini])", {
        "code-format:format-code": {
          description: "Format the active file with whichever provider claims it.",
    ...
    // observeTextEditors fires synchronously for already-open editors, and
    // watchEditor needs this.subscriptions assigned by then.
    ...
    const bufferSubscriptions = new CompositeDisposable(
      ...
      buffer.onWillSave(() => {
        const editor = lumine.workspace
          .getTextEditors()
          .find((item) => item.getBuffer() === buffer);
        if (!editor) return;
  ...
  async formatOnType(editor, { changes }) {
    ...
    if (!shouldFormatOnType(change, editor)) return;
  ...
  listProviders(event) {
    const editor = event.currentTarget.getModel();
    // [registry key, service name suffix, label] — the suffix is what a
    // provider package puts after "code-format." in its manifest.
    const kinds = [
      ["range", "range", "Range"],
      ["file", "file", "File"],
      ["onType", "on-type", "On-type"],
      ["onSave", "on-save", "On-save"],
    ];
    const lines = ["### Active providers in this editor", ""];
    for (const [kind, , label] of kinds) {
      lines.push(`- ${label}: ${this.providers[kind].providersForEditor(editor).length}`);
    }
    const packagesBySuffix = new Map(kinds.map(([, suffix]) => [suffix, []]));
    for (const pack of lumine.packages.getActivePackages()) {
      for (const name of Object.keys(pack.metadata?.providedServices ?? {})) {
        ...
        packagesBySuffix.get(name.slice("code-format.".length))?.push(pack.name);
      }
    }
    for (const [, suffix, label] of kinds) {
      const packages = packagesBySuffix.get(suffix);
      if (packages.length === 0) continue;
      lines.push("", `### ${label} formatters`, "", ...packages.map((name) => `- ${name}`));
    }
